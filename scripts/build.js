...
function getDeploymentDomain() {
  ...
  if (process.env.REPLIT_INTERNAL_APP_DOMAIN) {
    domain = stripProtocol(process.env.REPLIT_INTERNAL_APP_DOMAIN);
  } else if (process.env.REPLIT_DEV_DOMAIN) {
    domain = stripProtocol(process.env.REPLIT_DEV_DOMAIN);
  } else if (process.env.EXPO_PUBLIC_DOMAIN) {
    domain = stripProtocol(process.env.EXPO_PUBLIC_DOMAIN);
  } else {
    console.error(
  ...
  // Add port 5000 to development domains for Expo Go
  // This routes to Express (with static builds) instead of Metro (port 80 → 8081)
  if (domain && !domain.includes(":") && domain.includes("replit.dev")) {
    domain = `${domain}:5000`;
  }
...
function updateManifests(manifests, timestamp, baseUrl, assetsByHash) {
  const updateForPlatform = (platform, manifest) => {
    ...
    manifest.extra.expoClient.hostUri = baseUrl.replace("https://", "");
    manifest.extra.expoGo.debuggerHost = baseUrl.replace("https://", "");
